functions {
  ...
  real vecchia_lpdf(vector Z, vector sigma, real phi, real sigma_e, 
      ...
      for (j in 1:number_of_neigh) {
        for (k in j:number_of_neigh) {  // Only upper triangle
          ...
          // Sum over all covariance components
               for (l in 1:p) {
            cov_val += sigma[l] * (exp(-dist_jk/phi)) * A[l, neigh[j], neigh[k]];
          }
...
parameters {
  ...
  real<lower=0.01> phi;  // Bounded range for better mixing/ fixed
